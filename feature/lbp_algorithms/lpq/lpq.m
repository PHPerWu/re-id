function LPQhist = lpq(img,winSize,decorr,region_y,region_x,freqestim,mode)
...
% the region_y and region_x params
if nargin<5 || (isempty(region_y)&&isempty(region_x))
    region_y = 7;
    region_x = 6;
end 

% Local frequency estimation (Frequency points used [alpha,0], [0,alpha], [alpha,alpha], and [alpha,-alpha]) 
if nargin<6 || isempty(freqestim)
...
if strcmp(mode,'nh') || strcmp(mode,'h')
    for i = 1:region_y
        for j = 1:region_x
            region_result = LPQdesc((1+(i-1)*floor(r1/region_y)):i*floor(r1/region_y),(1+(j-1)*floor(c1/region_x)):j*floor(c1/region_x));
%             region_hist = hist(region_result(:),0:255) * region_weights(i,j);
            region_hist = hist(region_result(:),0:255);
            LPQhist = [LPQhist region_hist];
        end
    end
