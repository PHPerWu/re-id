...
function value = dlbpK(varargin) % image,radius,neighbors,mapping,mode)
% Version 0.3.2
...
% Determining the number of dominant patterns of DLBP
pattern = (dy+1)*(dx+1)*0.8;
pattern_sum = 0;
result = sort(result, 'descend');
for k = 1:length(result)
    pattern_sum = pattern_sum + result(k);
    if pattern_sum >= pattern
        break;
    end
end
